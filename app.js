...
// Edits to built-in categories (icon/color/label) are stored separately so
// the "builtin" flag (can't-delete) is preserved regardless of edits.
function loadCategoryOverrides() {
  try {
    return JSON.parse(localStorage.getItem('insight_category_overrides') || '{}');
  } catch { return {}; }
}
function saveCategoryOverrides(obj) {
  localStorage.setItem('insight_category_overrides', JSON.stringify(obj));
}
function getAllCategories() {
  const merged = Object.assign({}, BUILTIN_CATEGORIES);
  const overrides = loadCategoryOverrides();
  Object.keys(overrides).forEach(key => {
    if (merged[key]) merged[key] = Object.assign({}, merged[key], overrides[key]);
  });
  return Object.assign(merged, loadCustomCategories());
}

// ── PEOPLE (responsible / owner) ─────────────────────────────────────────────
function loadPeople() {
  try {
    return JSON.parse(localStorage.getItem('insight_people') || '[]');
  } catch { return []; }
}
function savePeople(arr) {
  localStorage.setItem('insight_people', JSON.stringify(arr));
}
...
function removePerson(name) {
  savePeople(loadPeople().filter(p => p !== name));
}
...
function populateOwnerSelect() {
  const sel = document.getElementById('event-owner');
...
function openEventForm(ctx) {
  ...
  document.getElementById('event-owner').value = isEdit ? (ctx.data.owner || '') : '';
...
async function submitEventForm() {
  ...
  const owner = document.getElementById('event-owner').value;
  ...
  if (formContext.mode === 'add') {
    await addEvent({ year: formContext.year, month: formContext.month, day: formContext.day, en1, en2, ar, category, owner, size });
  } else {
    await updateEvent(formContext.id, { en1, en2, ar, category, owner, size });
  }
  closeEventForm();
}
...
function saveEditedCalendar() {
  ...
  if (isBuiltin) {
    const overrides = loadCategoryOverrides();
    overrides[key] = { icon, label, color };
    saveCategoryOverrides(overrides);
  } else {
    const custom = loadCustomCategories();
    if (custom[key]) {
      custom[key] = Object.assign({}, custom[key], { icon, label, color });
...
function matchesOwner(ev) { return ownerFilter === 'all' || (ev.owner || '') === ownerFilter; }
...
function buildChip(ev) {
  ...
  div.innerHTML = `
    ...
    <span class="chip-meta-row">
      ${ev.owner ? `<span class="chip-owner-tag">👤 ${escapeHtml(ev.owner)}</span>` : ''}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('add-person-btn').addEventListener('click', () => {
    ...
    if (!addPerson(name)) { showToast('That person already exists.', true); return; }
    input.value = '';
    renderPeopleList();
    rebuildOwnerFilter();
